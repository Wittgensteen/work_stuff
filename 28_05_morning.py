    ...
    [
        html.Div(
            [
                dcc.Checklist(
                    id='interface',
                    options=[{'label': i, 'value': j} for i, j in zip(list_of_columns_for_gui, list_of_columns)],
...
def add_dropdown_month(value):
    try:
        ...
        if 'Month' not in value:
            child_mo = html.Div(style={'display': 'none'}),
...
@app.callback(dash.dependencies.Output('drop_month', 'children'),
              [dash.dependencies.Input('interface', 'values')
               ])
def update_drop_mon(val):
    print(val)
    print('Вход в функцию')
    children = add_dropdown_month(val)
    return children

def add_dropdown_owner(value):
    try:
        if 'Owner' in value:
            child_own = html.Div(
                 [
        ...
        if 'Owner' not in value:
            child_own = html.Div(style={'display': 'none'}),
        # else:
        #     child = html.Div(style={'display': 'none'}),
    except Exception as e:
        print(e)
...
@app.callback(dash.dependencies.Output('drop_owner', 'children'),
              [dash.dependencies.Input('interface', 'values')
               ])
def update_drop(val):
    print(val)
    print('Вход в функцию')
    children = add_dropdown_owner(val)
    return children
